...
import { convertEmailToPdf } from '../convertEmailToPdf'
...
const sliceCtxStub = { drawImage: vi.fn(), getImageData: vi.fn(() => ({ data: new Uint8ClampedArray(0) })) }
beforeAll(() => {
  vi.spyOn(HTMLCanvasElement.prototype, 'getContext').mockReturnValue(sliceCtxStub as unknown as CanvasRenderingContext2D)
  vi.spyOn(HTMLCanvasElement.prototype, 'toDataURL').mockReturnValue('data:image/jpeg;base64,SLICE')
})
...
describe('convertEmailToPdf', () => {
  ...
  it('creates a single-page PDF for short content', async () => {
    vi.mocked(html2canvas).mockResolvedValueOnce(makeCanvas(1588, 500) as unknown as HTMLCanvasElement)
    await convertEmailToPdf(makeEmail())
  ...
  it('creates a multi-page PDF for tall content (3 extra pages for 4x height)', async () => {
    vi.mocked(html2canvas).mockResolvedValueOnce(makeCanvas(1588, 4 * 2245) as unknown as HTMLCanvasElement)
    await convertEmailToPdf(makeEmail())
  ...
  it('handles multiple images in a single email body', async () => {
    const email = makeEmail({
      bodyHtml: '<img src="cid:a@e.com"><img src="https://ext.example.com/b.png"><img src="cid:c@e.com">',
      cidMap: {
