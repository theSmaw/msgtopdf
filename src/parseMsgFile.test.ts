...
describe('parseMsgFile', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(MsgReader).mockImplementation(
      () =>
        ...
        }) as unknown as InstanceType<typeof MsgReader>
    );
  });
  ...
  it('detects windows-1252 charset from meta tag and decodes correctly', async () => {
    const html =
      '<html><head><meta charset="windows-1252"></head><body><p>Charset test</p></body></html>';
  ...
  it('falls back to data.bodyHtml string when data.html is absent', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({ html: new Uint8Array(0), bodyHtml: '<p>Fallback HTML</p>' })
    );
    const result = await parseMsgFile(makeFile());
  ...
  it('falls back to data.bodyHtml when data.html is undefined', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({ html: undefined, bodyHtml: '<p>String body</p>' })
    );
    const result = await parseMsgFile(makeFile());
  ...
  it('falls back to data.body plain text, wrapping in <pre>', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({ html: new Uint8Array(0), bodyHtml: '', body: 'Plain text body' })
    );
    const result = await parseMsgFile(makeFile());
  ...
  it('escapes HTML in plain text body', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({ html: new Uint8Array(0), bodyHtml: '', body: '<b>bold</b>' })
    );
    const result = await parseMsgFile(makeFile());
    ...
    mockGetFileData.mockReturnValue(
      makeMsgData({
        html: new TextEncoder().encode(
          '<html><head><title>Title</title></head><body><p>Body only</p></body></html>'
        ),
      })
    );
    const result = await parseMsgFile(makeFile());
  ...
  it('strips inline event handlers (onclick, onload)', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({
        html: new TextEncoder().encode(
          '<body><a onclick="evil()">click</a><img onload="evil2()"/></body>'
        ),
      })
    );
    const result = await parseMsgFile(makeFile());
  ...
  it('strips <style> blocks', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({
        html: new TextEncoder().encode(
          '<body><style>.evil{display:none}</style><p>Content</p></body>'
        ),
      })
    );
    const result = await parseMsgFile(makeFile());
  ...
  it('strips <link> tags', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({
        html: new TextEncoder().encode(
          '<body><link rel="stylesheet" href="evil.css"><p>Content</p></body>'
        ),
      })
    );
    const result = await parseMsgFile(makeFile());
  ...
  it('formats sender as email only when name is absent', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({ senderName: '', senderEmail: 'john@example.com' })
    );
    const result = await parseMsgFile(makeFile());
  ...
  it('treats recipient with no recipType as a To recipient', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({
        recipients: [
          { recipType: undefined, name: 'NoType', email: 'notype@example.com', smtpAddress: '' },
        ],
      })
    );
    const result = await parseMsgFile(makeFile());
  ...
  it('builds cidMap entry for attachment with pidContentId', async () => {
    ...
    mockGetFileData.mockReturnValue(
      makeMsgData({
        attachments: [
          { pidContentId: 'img001@example.com', extension: '.png', fileName: 'img.png' },
        ],
      })
    );
  ...
  it('maps JPEG attachments with the correct MIME type', async () => {
    ...
    mockGetFileData.mockReturnValue(
      makeMsgData({
        attachments: [
          { pidContentId: 'photo@example.com', extension: '.jpg', fileName: 'photo.jpg' },
        ],
      })
    );
  ...
  it('maps .jpeg extension as image/jpeg', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({
        attachments: [
          { pidContentId: 'photo2@example.com', extension: '.jpeg', fileName: 'photo.jpeg' },
        ],
      })
    );
  ...
  it('defaults to image/png for unknown attachment extensions', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({
        attachments: [
          { pidContentId: 'unknown@example.com', extension: '.xyz', fileName: 'unknown.xyz' },
        ],
      })
    );
  ...
  it('skips attachment gracefully when getAttachment throws', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({
        attachments: [
          { pidContentId: 'broken@example.com', extension: '.png', fileName: 'broken.png' },
        ],
      })
    );
  ...
  it('derives extension from fileName when extension field is missing', async () => {
    mockGetFileData.mockReturnValue(
      makeMsgData({
        attachments: [
          { pidContentId: 'gif@example.com', extension: undefined, fileName: 'anim.gif' },
        ],
      })
    );
